...
public:
	...
	auto call(__A__ args) {
		Error().call(Less().call(len().call(args), Number(1)), String("No argument supplied"));
		Error().call(Not().call(IsNumber().call(Index().call(args, Number(0)))), String("Argument must be an integer"));
		Error().call(Less().call(StrToNum().call(Index().call(args, Number(0))), Number(1)), String("Argument must be above 0"));
